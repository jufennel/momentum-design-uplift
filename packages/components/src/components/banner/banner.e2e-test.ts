...
test('mdc-banner', async ({ componentsPage }) => {
  ...
  await test.step('visual-regression', async () => {
    ...
    const variantConfigs = [
      ...
      },
      {
        ...
        extraButtons:
          '<mdc-button variant="tertiary" prefix-icon="reset-bold" size="20" aria-label="Retry connection"></mdc-button>',
      },
      {
        variant: BANNER_VARIANT.SUCCESS,
        label: 'Changes Saved',
    ...
    const createVariantBanner = async (config: (typeof variantConfigs)[0]) => {
      ...
      const children = (config.extraChildren || '') + createDefaultActionButtons(config.extraButtons || '');
    ...
    bannerSheet.setAttributes({
      variant: BANNER_VARIANT.INFORMATIONAL,
      label:
        'This is an extremely long label that should wrap to multiple lines when the banner has a constrained width',
      'secondary-label':
        'This is also a very long secondary label with extensive descriptive text that should wrap appropriately',
  ...
  await test.step('attributes', async () => {
    ...
    await test.step('banner accepts allowed variant values', async () => {
      ...
      const testVariant = async (testCase: {
        variant: (typeof BANNER_VARIANT)[keyof typeof BANNER_VARIANT];
        expectIcon: boolean;
        name: string;
      }) => {
  ...
  await test.step('mouse interactions', async () => {
    await test.step('click on trailing action buttons (interactive elements)', async () => {
      ...
      const waitForPrimaryClick = await componentsPage.waitForEvent(primaryButton, 'click');
      await primaryButton.click();
      await expect(waitForPrimaryClick).toEventEmitted();

      const waitForClose = await componentsPage.waitForEvent(closeButton, 'click');
      await closeButton.click();
      await expect(waitForClose).toEventEmitted();
    });
  });
  ...
  await test.step('keyboard interactions', async () => {
    await test.step('user activates custom actions using keyboard', async () => {
      ...
      const waitForClickAfterEnter = await componentsPage.waitForEvent(actionButton, 'click');
